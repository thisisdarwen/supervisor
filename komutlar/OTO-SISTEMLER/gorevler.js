...
exports.run = async (client, message, args) => {
  ...
  if (sec == "bilgi") {
    ...
    if (args[1] !== "günlük" && args[1] !== "haftalık" && args[1] !== "aylık" ) message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
          ...
          if (!görev || görev.length === 0) return message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
