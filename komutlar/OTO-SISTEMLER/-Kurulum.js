...
exports.run = async (client, message, args) => {
  ...
  if (gkv.some(i => i == message.author.id) || message.author.id === message.guild.ownerID) {
    const sec = args[0]
    let emoji = message.guild.emojis.cache.find(x => x.name === "darwen_acik")
    let emoji2 = message.guild.emojis.cache.find(x => x.name === "darwen_kapali");
    let emoji3 = message.guild.emojis.cache.find(x => x.name === "darwen_tik")
