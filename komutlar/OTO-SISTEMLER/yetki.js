  ...
  if (data ? message.member.roles.cache.get(data.filter(x => message.member.roles.cache.map(y => y == x.YetkiliRol)).map(x => x.YetkiliRol)[0]) : message.member.permissions.has(8) || message.member.permissions.has(8) || message.guild.members.cache.some(x => client.ayarlar.yapimci.some(y => y == x.id))) {
    ...
    if (sec == "ayarla") {
      if (message.guild.members.cache.some(x => client.ayarlar.yapimci.some(y => y == x.id)) || message.author.id === message.guild.owner.id) {
        let sec2 = args[1]
        let sec3 = args[2]
        if ((await db.get(`yetkili_al.`) && await db.get(`yetkili_al.`).find(x => x.YetkiAdi === args[1]))) return message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
        if (sec2) {
          if (sec3) {
            let roles;
            ...
            message.react(`${client.emojis.cache.find(x => x.name === "darwen_onay")}`);
          } else return message.reply("**Doğru Kullanım:** `!yetki ayarla yetkiAdı <YetkiliRolID> @VerilecekRol @VerilecekRol2`"), message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);

        } else return message.reply("**Doğru Kullanım:** `!yetki ayarla yetkiAdı <YetkiliRolID> @VerilecekRol @VerilecekRol2`"), message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
      }
      else return message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
    }
    if (sec == "sil") {
      if (message.guild.members.cache.some(x => client.ayarlar.yapimci.some(y => y == x.id)) || message.author.id === message.guild.owner.id) {
        let sec2 = args[1]
        if (sec2) {
            ...
            message.react(`${client.emojis.cache.find(x => x.name === "darwen_onay")}`);
        } else return message.reply("**Doğru Kullanım:** `!yetki sil yetkiAdı`"), message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
      }
      else return message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
    }
...
`))
    ...
    if (sec == "ver") {
      ...
      if (data.map(x => x.YetkiAdi).some(i => i.includes(args[1].toLowerCase()))) {
        if (message.member.roles.cache.get(data.filter(x => message.member.roles.cache.map(y => y == x.YetkiliRol)).map(x => x.YetkiliRol)[0]) || message.member.permissions.has(8)) {
          ...
          return message.react(`${client.emojis.cache.find(x => x.name === "darwen_onay")}`)
        }
        return message.reply("Bu komut için gerekli izine sahip değilsin"), message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
      } return mesaj(data, message, client); 
    }
    ...
    if (sec == "al") {
      if (message.member.roles.cache.get(data.filter(x => message.member.roles.cache.map(y => y == x.YetkiliRol)).map(x => x.YetkiliRol)[0]) || message.member.permissions.has(8)) {
        if (!target) return message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
    ...
    return message.react(`${client.emojis.cache.find(x => x.name === "darwen_iptal")}`);
  }
...
Örnek kullanım:
\`${client.ayarlar.prefix[0]} yetki ver yetkiAdı @darwen#0001\`
\`${client.ayarlar.prefix[0]} yetki al @darwen#0001\`
...
\`${client.ayarlar.prefix[0]} yetki bilgi @darwen#0001\``: ""}
`);
